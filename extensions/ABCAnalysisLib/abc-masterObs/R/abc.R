...
abc <- function(target, param, sumstat, popFact, tol, method, hcorr = TRUE,
    ...
    #print("da INIzio abcyyp")
    #print(length(sumstat))
    #print(length(popFact))
    ...
    if(is.data.frame(popFact)) popFact <- as.matrix(popFact)
    if(is.vector(popFact)) popFact <- matrix(popFact, ncol=1)
    ...
    #print("sumstat length")
    #print(length(sumstat))
    
    #print("popFact length")
    #print(length(popFact))
    #print(popFact)
    #print(sumstat)
    ...
    #print(sumstat)
    #print(target)
    #print(scaled.sumstat)
    
    #ntarg<-length(targets)
    #ntargPop = ntarg/ncol(popFact)
    #print(popFact)
    ...
    for(j in 1:nss){
      ...
      if(target[j] == 0)
      {
        #sum1 <- sum1 + as.numeric(popFact[,j]) * ( ((scaled.sumstat[,j]-target[j])^2) )
        newS[,j] <-( ((scaled.sumstat[,j]-target[j])^2) )
        
      }
      else
      {
        newS[,j] <- ( ((scaled.sumstat[,j]-target[j])^2) / (target[j]^2) )
        #sum1 <- sum1 + as.numeric(popFact[,j]) * ( ((scaled.sumstat[,j]-target[j])^2) / (target[j]^2) )
      ...
      #print(j)
      #print(fact)
      #print("simulated not scaled")
      #print((scaled.sumstat[,j]))
      #print("target")
      #print(target[j])
      #print("pop")
      #print(popFact[ fact])
      #print("tmp")
      #print((scaled.sumstat[,j]-target[j])^2 / target[j]^2)
      #print("tmp * pop")
      #print((as.numeric(popFact[, fact]) * (scaled.sumstat[,j]-target[j])^2) / target[j]^2)
      #print("sum1")
      #print(sum1)
      #print("dist (sqrt(sum1))")
      #print(sqrt(sum1))
    ...
    if(is.data.frame(newSS)) param <- as.matrix(newSS)
    if(is.vector(newSS)) newSS <- matrix(newSS, ncol=1)
