...
cv4abc <- function(param, sumstat, popFact, abc.out = NULL, nval, tols,
                   statistic = "median", prior.range = NULL,
                   method, hcorr = TRUE,
                   transf = "none", logit.bounds = c(0,0), 
                   subset = NULL, kernel = "epanechnikov",
                   numnet = 10, sizenet = 5, 
    ...
    if(is.null(dim(popFact))){
        popFact <- as.data.frame(popFact)
    }
    ...
    for(mytol in tols){
        ...
        for(i in 1:nval){
            ...
            #print("i")
            #print(i)
            #print("mysumstat length")
            #print(length(sumstat))
            #print(length(mysumstat))
            
            #print("mypopFact length")
            #print(length(popFact))
            #print(length(mypopFact))
            
            subres <- withCallingHandlers( abc(target = mytarget, param = myparam, 
                                               sumstat = mysumstat, popFact = mypopFact,
                                               tol=mytol,
                          subset = mysubset, method = method, 
                          transf = transf, logit.bounds = logit.bounds, 
                          kernel = kernel,hcorr = hcorr), warning = namesWarningFilter)
        ...
        mycall[[paste("tol", mytol, sep="")]] <- call("abc", target = quote(target), param = quote(param), 
                                                      sumstat = quote(sumstat), popFact = quote(popFact),
                                                      tol= mytol,
